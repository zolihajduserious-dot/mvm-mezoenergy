...
foreach (mvm_pdf_overlay_fields($values) as $field) {
    if ((int) $field['page'] !== 1 || trim((string) $field['text']) === '') {
        continue;
    }
    $i++;
    echo 'before ' . $i . ' ' . $field['text'] . PHP_EOL;
    $path = mvm_pdf_text_image((string) $field['text'], (float) $field['w'], (float) $field['h'], (int) $field['size']);
    echo 'after ' . $i . ' ' . filesize($path) . PHP_EOL;
    unlink($path);
}
echo 'done ' . $i . PHP_EOL;
